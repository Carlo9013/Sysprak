...
void gameInfoFree(gameInfo *game, playerInfo *player){
    if (NULL != game->gameName) {
        free(game->gameName);
    }
    if (NULL != player->playerName) {
        free(player->playerName);
    }
    if (NULL != player->anotherPlayerName) {
    ...
    while ((opt = getopt(argc, argv, "g:p:c:")) != -1) {
        switch (opt) {
            ...
            case 'c':
                ...
                break;
            default:
                printf("ERROR: Please specify <GAME-ID> and <Player> with ./sysprak-client -g <GAME-ID> -p <Player>. -c <Config Address>\n");
                free(config_path);
                configInfoClear(&conf);
                return 1;
        }
